...
function start(){
    ...
    for(i = 0; i < lines.length; i++){
        ...
        if (line[0] !== '#') {
            ...
            dictionarysimplified[elements.simplified] = dictionarysimplified[elements.simplified] || [];
            dictionarysimplified[elements.simplified].push(elements);

            dictionarytraditional[elements.traditional] = dictionarytraditional[elements.traditional] || [];
            dictionarytraditional[elements.traditional].push(elements);
        }
    }
    ...
    function getElements(line){
        ...
        var defStart = line.indexOf('/');
        var defClose = line.lastIndexOf('/');
        var pinyin = line.substring(openbracket+1, closebracket);
        var definition = line.substring(defStart+1, defClose);
...
function definitionLookup(word, scripttype){
    if (scripttype === "s" || determineIfSimplified(word)) {
        return dictionarysimplified[word];
    }
    return dictionarytraditional[word];
}

function dictionarySearch(characters, type){
    /*--- Types: Only = Just the characters and no alternatives. If not then finds all cases of that character ---*/
    var search = [], regexString, regex;

    if(type == "only") {
        // This also finds 爸爸 when characters == "爸"
        regexString = "^(" + characters.split('').join("|") + ")+$";
    } else {
        regexString = "[" + characters + "]";
    }
...
function loadFrequencyData(){
    var readFile, lines, i, splits, word, freq;

    console.log("Starting to read frequency data");

    readFile = fs.readFileSync(__dirname + '/data/leidenfreqdata.txt', 'utf-8');
    ...
    readFile = fs.readFileSync(__dirname + '/data/frequencyjunda.txt', 'utf-8');
    lines = readFile.split(/\r?\n/);
...
function loadIrregularPhonetics(){
    var readFile, lines, i, splits, character, pinyin;

    readFile = fs.readFileSync(__dirname + '/data/irregularphonetics.txt', 'utf-8');
    lines = readFile.split(/\r?\n/);
    for(i=0; i<lines.length; i++){
        splits = lines[i].split(":");
        character = splits[0];
