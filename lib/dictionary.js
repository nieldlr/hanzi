...
function loadFrequencyData(){
	console.log("read");
	var	readFile = fs.readFileSync(__dirname + '/data/words_types.txt', 'utf-8');
	console.log("done");
	lines = readFile.split(/\r?\n/);
	...
	for(; i<lines.length; i++) {
		var splits = lines[i].split(",");
		var wordstr = splits[1];
		var firstcomma = wordstr.indexOf('"');
		var lastcomma = wordstr.lastIndexOf('"');
		var word = wordstr.substring(firstcomma+1, lastcomma);
		var freqstr = splits[3];
		firstcomma = freqstr.indexOf('"');
		lastcomma = freqstr.lastIndexOf('"');
		var freq = freqstr.substring(firstcomma+1, lastcomma);
		wordfreq[word] = freq;
	};

	//console.log(wordfreq['音乐']);
}
