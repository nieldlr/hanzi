...
function getLongestMatch(input_str, max_word_len) {
    var i = max_word_len ? max_word_len : 8;
    while (i >= 0) {
        var slice = input_str.substr(0, i);
        console.log("Checking slice ", slice)
        if (dictionarysimplified.hasOwnProperty(slice)) {
            return slice;
        }
        console.log("Checking slice ", slice)
        if (dictionarytraditional.hasOwnProperty(slice)) {
            return slice;
        }
        i--;
    }
...
function segment(input_str) {
    var seg, segments, split_chars, start;
    split_chars = [" ", '"', ";", ":", "!", ".", ",", "[", "]", "(", ")", "_", "-", "+", "=", " ", " ", "，", "‘", "“", "”", "。", "「", "」", "《", "》", "／", "？", "！", "&", "（", "）", "－", "—", "—", "＋", "＝", "、", "｜", "｀", "～"];
    segments = [];
    start = 0;
    while (start <= input_str.length) {
        seg = getLongestMatch(input_str.slice(start));
        segments.push(seg);
