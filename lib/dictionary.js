...
function getLongestMatch(input_str) {
    var max_word_len = 8
    var i = max_word_len > input_str.length ? max_word_len : input_str.length
    while (i >= 0) {
        var slice = input_str.substr(0, i);
        if (slice in dictionarysimplified) {
            return slice;
        }
        if (slice in dictionarytraditional) {
            return slice;
        }
        i--;
    }
...
function segment(input_str) {
    ...
    while (input_str.length > 0) {
        var seg = getLongestMatch(input_str);
        if (seg === null)   //if no match, just push one chr
