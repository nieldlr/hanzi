...
function getPinyin(character){
	//console.log("Checking for pinyin: " + character);
	//These are for components not found in CC-CEDICT.
	if("undefined" != typeof irregularphonetics[character]){
		//console.log("Irregular found: " + irregularphonetics[character]);
...
function determinePhoneticRegularity(decomposition){
		...
		for(; i<charpinyin.length; i++){ //Compare it with all the possible pronunciations of the character
			if(phoneticpinyin == null){ //If the component has no pronunciation found, nullify the regularity computation
				regularityarray[charpinyin[i]] = {
					...
					component: [decomposition.components1[k]],
					phoneticpinyin: [null],
					regularity: [null]
				};
			}
				...
				for(; j<phoneticpinyin.length; j++){
					if("undefined" == typeof regularityarray[charpinyin[i]]){ //If the object store has no pinyin stored yet, create the point
						regularityarray[charpinyin[i]] = {
							character: decomposition.character,
							component: [decomposition.components1[k]],
							phoneticpinyin: [phoneticpinyin[j]],
							regularity: [getRegularityScale(charpinyin[i], phoneticpinyin[j])]
						};
					}
					else{ //If there is one already, check to see if the current component is new or not
						if(regularityarray[charpinyin[i]].component != decomposition.components1[k]){ //If it is new, push to phoneticpinyin field & push regularity.
	...
	for(var k=0; k<decomposition.components2.length; k++){
		...
		for(; i<charpinyin.length; i++){ //Compare it with all the possible pronunciations of the character
			if(phoneticpinyin == null){ //If the component has no pronunciation found, nullify the regularity computation
				regularityarray[charpinyin[i]] = {
					character: decomposition.character,
					component: [decomposition.components2[k]],
					phoneticpinyin: [null],
					regularity: [null]
				};
			}
				...
				for(; j<phoneticpinyin.length; j++){
					if("undefined" == typeof regularityarray[charpinyin[i]]){ //If the object store has no pinyin stored yet, create the point
						...
						if(regularityarray[charpinyin[i]].component != decomposition.components2[k]){ //If it is new, push to phoneticpinyin field & push regularity.
							regularityarray[charpinyin[i]].phoneticpinyin.push(phoneticpinyin[j]);
	...
	//Determine phonetic regularity for components on level 2 decomposition
	/*for(var k=0; k<decomposition.components2.length; k++){
		phoneticpinyin = getPinyin(decomposition.components2[k]);
		if(phoneticpinyin == null){
			regularityarray.push({
				component: decomposition.components2[k],
				charpinyin: charpinyin[k],
				phoneticpinyin: null,
				regularity: null
			});
		}
	...
	//Remove surnames
	/*for(var q=0; q<regularityarray.length; q++){
		if(/[A-Z]/.test(regularityarray[q].phoneticpinyin)){
			regularityarray.splice(q,1);
		}
	}
	console.log(regularityarray);
	//Remove duplicates
	for(var q=0; q<regularityarray.length; q++){
		for(var r=0; r<regularityarray.length; r++){
			console.log(regularityarray[q].component + q);
			if(regularityarray[q].phoneticpinyin == regularityarray[r].phoneticpinyin && q != r){
				regularityarray.splice(r,1);
			}
		}
	}*/
