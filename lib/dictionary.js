...
function getPinyinUtf8(pinyins) {
  // tone placed on first priority vowel except for iu
  if (Array.isArray(pinyins) === true) {
    return pinyins.map(x =>
      x
        .split(' ')
        .map(y => toUtf8(y))
        .join(' ')
    ...
    return toUtf8(pinyins);
  }

  function toUtf8(apinyin) {
    var priority = ['a', 'e', 'i', 'o', 'u'];
    var pinyin = apinyin.replace('u:', 'v');
    var tone_level = pinyin[pinyin.length - 1];

    if (isNaN(String(tone_level) * 1) === true) {
      return pinyin;
    }
    ...
    for (var i = 0; i < pinyin.length; i++) {
      var c = pinyin[i];
      var is_char = 'aeiouv'.indexOf(c) !== -1;
      if (is_char === true) {
        var pic = priority.indexOf(c);
        var is_priority = pic < tone[0];
        if (is_priority === true) {
          tone = [pic, i];
        }
      }
    ...
    if (tone[1] !== -1) {
      result = pinyin.split('').slice(0, pinyin.length - 1);
      result[tone[1]] = vowel_by_tone[tone_level - 1][result[tone[1]]];
      return result.join('');
    } else {
      return pinyin;
    }
...
function getPinyin(character, utf8 = false) {
  if (utf8 == true) {
    return getPinyinUtf8(getPinyinAscii(character));
  } else {
    return getPinyinAscii(character);
  }
}

function getPinyinAscii(character) {
...
exports.getPinyinUtf8 = getPinyinUtf8;
