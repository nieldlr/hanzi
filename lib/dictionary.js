...
function determinePhoneticRegularity(decomposition){
	...
	if(null == charpinyin){
		return regularitytable[decomposition.character] = null;
	}

	var phoneticpinyin = [];
	var regularityarray = [];
	
	//Determine phonetic regularity for component on level 1 decomposition
	for(var k=0; k<decomposition.components1.length; k++){
		phoneticpinyin = getPinyin(decomposition.components1[k]);
		var i=0;
	...
	regularitytable[decomposition.character] = regularityarray;
	return regularitytable;
