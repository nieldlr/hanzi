...
function getRegularityScale(charpinyin, phoneticpinyin) {
  ...
  // rhyming (a 4 or a 5 whose initials also share a place of articulation, e.g.
  // 编 bian / 篇 pian), 0 = No regularity
  ...
  // A matching final on its own can be coincidence; a matching final carried by
  // a near-identical initial rarely is, so that pairing outranks both.
  if (
    (samefinal || samerhyme) &&
    pinyin.sameInitialGroup(character.initial, phonetic.initial)
  ) {
    return 6;
  }
  if (samefinal) {
